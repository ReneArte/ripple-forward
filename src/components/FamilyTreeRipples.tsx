...
const FamilyTreeRipples: React.FC = () => {
  ...
  const getLevelColor = (level: number, isYou: boolean = false) => {
    ...
    switch (level) {
      ...
      case 2:
        return 'from-hope-green to-hope-green-dark';
      case 3:
        return 'from-ripple-teal to-blue-600';
      default:
  ...
  return <div className="py-16 px-4 bg-gradient-to-br from-green-50 to-blue-50 dark:from-slate-900 dark:to-indigo-900">
      <div className="container mx-auto">
        <h2 className="font-poppins font-semibold text-3xl md:text-4xl text-center mb-12">My Ripples</h2>
        
        <div className="flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
          {/* Family Tree Visualization */}
          <div className="flex-1">
            <AnimatedTree people={treePositions} selectedPerson={selectedPerson} onPersonSelect={setSelectedPerson} />
          ...
          <div className="w-full lg:w-80">
            {selectedPerson ? <Card className="sticky top-8">
                <CardContent className="p-6">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {selectedPerson.level === 0 ? 'Your nominator' : selectedPerson.level === 1 ? 'You' : selectedPerson.level === 2 ? 'Direct connection' : '2nd degree connection'}
                    </p>
                  </div>
                  
                  {selectedPerson.id !== 'you' && selectedPerson.paymentApps && selectedPerson.paymentApps.length > 0 && <div className="space-y-3">
                      <h4 className="font-medium text-sm text-muted-foreground mb-3">Support this person:</h4>
                      {selectedPerson.paymentApps.map(app => <Button key={app} variant="outline" className="w-full justify-start capitalize hover:bg-primary/5" onClick={() => {
                  ...
                  {selectedPerson.id !== 'you' && <div className="mt-6 pt-4 border-t border-border">
                      <Button className="w-full bg-hope-green hover:bg-hope-green-dark text-white" onClick={() => {
